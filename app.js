  ...
  if (svgPath) {
    fetch(encodeURI(svgPath))
      ...
      .then((svgText) => {
        ...
        const vb = viewBox && viewBox.width && viewBox.height
        ...
        const radius = Math.max(vb.width, vb.height) * 0.22;
        const states = paths.map((path, index) => {
          ...
          return {
            ...
            cx,
            cy,
            seed: (Math.sin(index * 2.14) + Math.cos(index * 0.77)) * 0.5,
            tx: 0,
            ty: 0,
            rot: 0,
            targetTx: 0,
        ...
        const updateTargets = () => {
          states.forEach((item) => {
            ...
            item.targetTx = nx * force * 2.7 + item.seed * force * 0.45;
            item.targetTy = ny * force * 1.8 - item.seed * force * 0.35;
            item.targetRot = (nx * 2.6 - ny * 1.2 + item.seed * 3.2) * force;
          });
        };
        ...
        const render = () => {
          ...
          states.forEach((item) => {
            item.tx = lerp(item.tx, item.targetTx, 0.14);
            item.ty = lerp(item.ty, item.targetTy, 0.14);
            item.rot = lerp(item.rot, item.targetRot, 0.13);

            energy += Math.abs(item.tx) + Math.abs(item.ty) + Math.abs(item.rot);
            item.path.style.transform = `translate(${item.tx.toFixed(2)}px, ${item.ty.toFixed(2)}px) rotate(${item.rot.toFixed(2)}deg)`;
          });

          if (pointer.inside || energy > 0.08) {
            rafId = window.requestAnimationFrame(render);
          } else {
            rafId = 0;
            states.forEach((item) => {
              item.path.style.transform = "";
            });
          }
        };
        ...
        const updatePointer = (event) => {
          ...
          pointer.x = vb.x + nx * vb.width;
          pointer.y = vb.y + ny * vb.height;
          pointer.inside = true;
          updateTargets();
          ensureRender();
        };

        inlineLogoWrap.addEventListener("pointerenter", updatePointer, { passive: true });
        inlineLogoWrap.addEventListener("pointermove", updatePointer, { passive: true });
        inlineLogoWrap.addEventListener("pointerleave", () => {
          pointer.inside = false;
          resetTargets();
          ensureRender();
        });
        inlineLogoWrap.addEventListener("pointercancel", () => {
          pointer.inside = false;
          resetTargets();
          ensureRender();
        });
